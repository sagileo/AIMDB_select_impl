...
class Catalog {
  private:
    ...
    std::tr1::unordered_map <std::string, Object *> cl_name_obj;    /**< name map of Objects ,name should not be the same in the whole Catalog */
  ...
  public:
	 ...
	 * @param t_id		  reference of the table identifier where this column belongs
     * @retval true       success
     * @retval false      failure
     */
    bool createColumn(const char *name, ColumnType type,int64_t option_size, int64_t & c_id, int64_t & t_id);
    /**
     * create index.
  ...
  private:
     ...
     * @param  t_id  index in which table
     * @retval true  success
     * @retval false failure
     */
    ...
    int64_t registerObj(Object * obj) {
        ...
        cl_name_obj.insert(std::pair < std::string,
                           Object * >(std::string(obj->getOname()), obj));
        return obj->getOid();
    }
    /**
