...
Project::Project(SelectQuery *query) {
	...
	{
		sel_t_id[i] = ((Column*)(g_catalog.getObjByName(query->select_column[i].name)))->getTid();
...
Join::Join(Project *parent, int from_number, RequestTable *from_table, Conditions *where){
	...
	for(int i = 0; i < where->condition_num; i++)
	{
		...
		t_id_0 = ((Column *)(g_catalog.getObjById(c_id_0)))->getTid();
		t_id_1 = ((Column *)(g_catalog.getObjById(c_id_1)))->getTid();
		tp0 = (RowTable*)(g_catalog.getObjById(t_id_0));
		tp1 = (RowTable*)(g_catalog.getObjById(t_id_1));

		if(getIndex(c_id_0) != -1)		
			goto assign1;
		else if(getIndex(c_id_1) != -1)
			goto assign0;
	...
	{
		int index;
		if((index = getIndex(this->c_id[i][0])) != -1)
		{
			has_index[i] = true;
			i_id[i] = index;
		}
