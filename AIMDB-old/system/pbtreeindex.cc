...
bool PbtreeIndex::set_ls(void *i_data1, void *i_data2, void *info) {
    ...
    if (i_data2) {
        ...
        pb_info->le_resu = pi_pbtree.get_recptr (pb_info->l_ptr,pb_info->result,PBTREEINFO_CAPACITY,(pb_info->pos_resu));
    }
    pb_info->cr_area = 0;
    pb_info->cr_resu = 0;
    return true;
...
bool PbtreeIndex::scan (void *info, void *&result) {
    ...
    while (1) {
        ...
        pb_info->le_resu = pi_pbtree.get_recptr (pb_info->area[pb_info->cr_area],pb_info->result,PBTREEINFO_CAPACITY,pb_info->pos_resu);
        if (pb_info->le_resu) {
            pb_info->cr_resu = 0;
            continue;
        }
        ...
        pb_info->s_num = PBTREEINFO_CAPACITY;
